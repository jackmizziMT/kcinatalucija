-- Fixed Supabase Schema for Inventory Tracker
-- This version handles existing tables and permissions properly
...
-- Create app_users table (renamed to avoid conflict with auth.users)
...
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.locations;
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.stock_by_location;
DROP POLICY IF EXISTS "Allow all operations for authenticated users" ON public.audit_trail;
...
DROP TRIGGER IF EXISTS update_stock_by_location_updated_at ON public.stock_by_location;
